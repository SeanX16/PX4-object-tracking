...
void doImg(const sensor_msgs::Image::ConstPtr &msg) {
    ...
    // 将无人机发布的图像转为 BGR8 格式，然后转换为灰度图，再二值化，就能得到黑白图像，若小车出现，那么在图像内有黑色的像素，否则图像全是白色像素，这也是我将小车改成黑色的原因，若改成其它颜色就不好进行分离
    ...
    // 对二值图像进行形态学开运算来去除噪声
    cv::Mat bin_filtered;
    cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3)); // 3x3 的矩形内核
    ...
    // 使用 cv::findContours 提取所有轮廓（假设目标为黑色，背景为白色）
    std::vector<std::vector<cv::Point>> contours;
    std::vector<cv::Vec4i> hierarchy;
    cv::findContours(bin, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

    // 如果找到了轮廓，则选择最大的一个作为目标
    if (!contours.empty()) {
        // 找到最大轮廓（依据轮廓面积）
