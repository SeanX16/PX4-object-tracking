...
void doImg(const sensor_msgs::Image::ConstPtr &msg) {
    ...
    // Perform morphological opening on the binary image to remove noise
    cv::Mat bin_filtered;
    cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3)); // 3x3 rectangular kernel
    cv::morphologyEx(bin, bin_filtered, cv::MORPH_OPEN, kernel);

    // Get the image width and height
    static int row = bin.rows, col = bin.cols;
    // Image center position. We assume the UAV is at the center of the image,
    // which makes it easier to publish velocity commands to control the drone
    static double centX = row / 2, centY = col / 2;

    // x and y store the position where the car appears in this frame
    int x, y;
    // Flag to indicate whether the car is found
    bool findCar = false;

    // Use cv::findContours to extract all contours (assuming the target is black and background is white)
    std::vector<std::vector<cv::Point>> contours;
    std::vector<cv::Vec4i> hierarchy;
    cv::findContours(bin, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

    // If contours are found, select the largest one as the target
    if (!contours.empty()) {
        ...
        // Compute image moments of the contour to get its centroid
